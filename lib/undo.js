(function() {
  ...
  module.exports = Undo = (function() {
    function Undo(evaluator, game) {
      ...
      this.game = game;
      this.backup = new Array((this.game.n * 3 + 2 + 1 + this.game.nn) * this.game.maximum_search_depth);
      this.top_ndx = 0;
    }
    Undo.prototype.mark = function() {
      ...
      this.backup[this.top_ndx++] = this.game.zobrist_b;
      this.backup[this.top_ndx++] = this.game.zobrist_c;
      this.backup[this.top_ndx++] = this.game.current_player;
      this.backup[this.top_ndx++] = this.evaluator.manhattan_balance;
      i = 0;
    ...
    Undo.prototype.restore_last_mark = function() {
      ...
      this.evaluator.manhattan_balance = this.backup[--this.top_ndx];
      this.game.current_player = this.backup[--this.top_ndx];
      this.game.zobrist_c = this.backup[--this.top_ndx];
      this.game.zobrist_b = this.backup[--this.top_ndx];
