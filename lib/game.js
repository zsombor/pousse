(function() {
  var Evaluator, Game, GameHistory, MoveList, Readline, TranspositionTable, Undo, max_value, positional_value, square, square_sides;
  ...
  module.exports = Game = (function() {
    ...
    Game.prototype.store_best_move = function(value, beta, depth, best_move) {
      ...
      if (depth === this.current_iteration_depth) {
        console.log("'" + best_move.side + " " + best_move.ndx + "' seems most promissing at depth " + depth + " after processed " + this.nr_processed_nodes + " nodes with " + this.transposition_table_hits + " tt hits");
      ...
      handle_input = __bind(function(chunk) {
        ...
        if (match) {
          ...
          if (ndx >= 0 && ndx < this.n) {
            ...
            if (this.evaluator.is_game_over()) {
              console.log("Game over!");
              console.log(this.evaluator.print_balances());
              console.log(this.history.loop_encountered);
              process.exit(0);
            }
          } else {
